...
const PORT = 3000;
  ...
  // Dedicated YouTube Restricted Mode & Restricted Wi-Fi Bypass Embed Proxy Route
  app.get(["/api/youtube/embed", "/api/yt/embed"], (req, res) => {
    let videoId = (req.query.v as string) || (req.query.id as string) || (req.query.videoId as string) || "";
    ...
    if (!videoId) {
      return res.status(400).send("Missing YouTube video ID or URL");
    }
...
<head>
  ...
  <title>YouTube Player (Unrestricted)</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .mirror-btn:hover, .mirror-btn.active {
      background: #3b82f6;
      color: #fff;
    }
  </style>
...
<body>
  <div id="player-container">
    <div id="mirror-bar">
      <button class="mirror-btn active" onclick="switchMirror('nocookie', this)">No-Cookie (Fast)</button>
      <button class="mirror-btn" onclick="switchMirror('invidious1', this)">Mirror A (Bypass)</button>
      <button class="mirror-btn" onclick="switchMirror('invidious2', this)">Mirror B (Unblock)</button>
    </div>
    <iframe id="main-iframe" 
      src="https://www.youtube-nocookie.com/embed/${videoId}?autoplay=0&modestbranding=1&rel=0&playsinline=1" 
  ...
  <script>
    ...
    const mirrors = {
      nocookie: "https://www.youtube-nocookie.com/embed/" + videoId + "?autoplay=1&modestbranding=1&rel=0&playsinline=1",
      invidious1: "https://yewtu.be/embed/" + videoId + "?autoplay=1",
      invidious2: "https://invidious.nerdvpn.de/embed/" + videoId + "?autoplay=1"
    };
    function switchMirror(key, btn) {
      ...
      if (frame && mirrors[key]) {
        frame.src = mirrors[key];
      }
    }
  </script>
</body>
